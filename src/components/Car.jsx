...
//   const FORCE = boostActive ? 13 : 11; // Increase force when boost is active
...
export function Car(props) {
  ...
  const resetCar = () => {
    if (rigidBody.current) {
      rigidBody.current.setTranslation({ x: 10, y: 10, z: 0 }); // Reset position
      const uprightRotation = new THREE.Quaternion(); // Identity quaternion (no rotation, upright)
      uprightRotation.setFromEuler(new THREE.Euler(0, Math.PI / 2, Math.PI / 2)); // Rotate -90 degrees on X-axis
